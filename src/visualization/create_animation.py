...
def create_2D_animation(data, output_filepath):
    ...
    anim = animation.FuncAnimation(fig, animate_2D, interval=INTERVAL, 
                                    fargs=(im, data), 
                                    frames=num_frames)
...
def create_3D_animation(data, output_filepath):
    ...
    plot = [ax.plot_surface(X, Y, data[0,:,:], cmap=CMAP, linewidth=0, rstride=1, cstride=1)]

    anim = animation.FuncAnimation(fig, animate_3D, interval=INTERVAL,
