...
# Training Setup 
# Label for training
TRAIN_ID = "cnn-norm-circ"
# bool: Load pre-trained model
ReTrain = False 
# If Retrain == true: set the pre-trained model path
MODEL_LOAD_PATH = os.path.join(TRAINED_MODEL_DIR, 'RL/13.00.pt')
...
RENDER_EPOCH = 50   # store train history every () epochs
STOP_CRITERIA = 1000
...
def main():
    ...
    dataset = np.load(config.DATASET_FILEPATH[1])  
    print("dataset load successfully:", dataset.shape)
